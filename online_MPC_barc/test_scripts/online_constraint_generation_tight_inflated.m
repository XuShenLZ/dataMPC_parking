...
for i = start_k:T-N
    ...
    horizon_collision = [];
    coll_bound_x = cell(N+1,1);
    coll_bound_y = cell(N+1,1);
    for j = 1:N+1
        scaling = max(1, 2*abs(TV_v(j))/v_ref);
%         scaling = 5;
        vel_dir = sign(TV_v(j));
        
        phi = linspace(0, 2*pi, 200);
        coll_bound_x{j} = zeros(1, 200);
        coll_bound_y{j} = zeros(1, 200);
        for k = 1:length(phi)
            [x_b, y_b, ~, ~] = get_collision_boundary_point_tight_inflated(0, 0, phi(k), TV.width, TV.length, r, vel_dir, scaling);
            coll_bound_x{j}(k) = x_b;
            coll_bound_y{j}(k) = y_b;
        end
    
        ref = [EV_x_ref(j); EV_y_ref(j)];
        collision = check_collision_point_inflated(ref, TV_x(j), TV_y(j), TV_th(j), TV.width, TV.length, r, vel_dir, scaling);
...
%             s = 1;
    ...
    coll_bound_global = R(TV_th(1))*[coll_bound_x{1}; coll_bound_y{1}] + [TV_x(1); TV_y(1)];
    l_TV = [l_TV plot(coll_bound_global(1,:), coll_bound_global(2,:), 'color', cmap(1,:))];
    ...
    for j = 1:N+1
        ...
        if ~isempty(hyp(j).w)
%             coll_bound_global = R(TV_th(j))*[coll_bound_x{j}; coll_bound_y{j}] + [TV_x(j); TV_y(j)];
