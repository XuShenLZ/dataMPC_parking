...
function [col, T_final] = FSM_HOBCA_strat_fp_inflated_parameterized(exp_num, data_gen)
    ...
    ws_params.name = 'ws_solver_strat';
    ws_params.N = N;
    ws_params.n_x = n_z;
    ws_params.n_u = n_u;
    ...
    opt_params.name = 'opt_solver_strat';
    opt_params.N = N;
    opt_params.n_x = n_z;
    opt_params.n_u = n_u;
    ...
    if ~exist('forces_pro_gen_strat', 'dir')
        mkdir('forces_pro_gen_strat')
    end
    cd forces_pro_gen_strat
    obca_controller = hpp_obca_controller_FP_parameterized(true, ws_params, opt_params);
    cd ..
    ...
    while ~isequal(FSM.state, "End")
        ...
        if any(FSM.state == ["Safe-Confidence", "Safe-Yield", "Safe-Infeasible"]) || EV_v*cos(EV_th) <= 0
            rel_state = [(TV_x-EV_x)/scaling_factor; 
                        (TV_y-EV_y)/scaling_factor; 
                        (TV_th-EV_th); 
                        (TV_v.*cos(TV_th)-0)/scaling_factor; 
                        (TV_v.*sin(TV_th)-EV_v*sin(EV_th))/scaling_factor];
        else
            rel_state = [(TV_x-EV_x)/scaling_factor; 
                        (TV_y-EV_y)/scaling_factor; 
                        (TV_th-EV_th); 
                        (TV_v.*cos(TV_th)-EV_v*cos(EV_th))/scaling_factor; 
                        (TV_v.*sin(TV_th)-EV_v*sin(EV_th))/scaling_factor];
        end
