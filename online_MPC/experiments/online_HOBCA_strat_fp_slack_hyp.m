...
opt_params.name = 'FP_opt_solver_strat_slack_hyp';
opt_params.N = N;
opt_params.n_x = n_z;
opt_params.n_u = n_u;
...
cd forces_pro_gen
obca_controller = hpp_obca_controller_FP_slack_hyp(false, ws_params, opt_params);
cd ..
...
%     tv_obs(1,:) = get_car_poly_obs(TV_pred, TV.width, TV.length);
    car_obs = get_car_poly_obs(TV_pred, TV.width, TV.length);
