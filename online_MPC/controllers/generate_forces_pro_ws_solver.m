function generate_forces_pro_ws_solver(params) 
    global n_x n_obs n_ineq d_ineq m_ineq G g 
    ...
    % [lambda, mu, d]
    ws_model.nvar = n_obs*n_ineq + n_obs*m_ineq + n_obs;
    ws_model.ub = [inf*ones(n_obs*n_ineq+n_obs*m_ineq,1); inf*ones(n_obs,1)];
    ws_model.lb = [zeros(n_obs*n_ineq+n_obs*m_ineq,1); -inf*ones(n_obs,1)];
...
function ws_obj = eval_ws_obj(z)
    global n_obs n_ineq m_ineq
    
    d = z(n_obs*n_ineq+n_obs*m_ineq+1:end);
    ws_obj = -sum(d);
end
...
function ws_eq = eval_ws_eq(z, p)
    ...
    for i = 1:n_obs
        ...
        mu = z(n_obs*n_ineq+(i-1)*m_ineq+1:n_obs*n_ineq+i*m_ineq);
        d = z(n_obs*n_ineq+n_obs*m_ineq+i);
